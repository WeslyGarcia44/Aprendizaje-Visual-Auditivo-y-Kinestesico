document.addEventListener("DOMContentLoaded", () => {
    ...
    // Determinar si el ejercicio es con posiciones aleatorias
    const isRandomVariant = dropZone.classList.contains("random-variant");
    ...
    // Configuración para la variante aleatoria
    if (isRandomVariant) {
        const dropTargets = dropZone.querySelectorAll("[data-id]");

        // Posicionar las zonas de soltar aleatoriamente
        dropTargets.forEach(target => {
            const x = Math.random() * (window.innerWidth - 100); // Mantener margen de 100px
            const y = Math.random() * (window.innerHeight - 100); // Mantener margen de 100px
